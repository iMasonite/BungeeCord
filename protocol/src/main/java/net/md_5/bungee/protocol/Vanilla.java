...
import net.md_5.bungee.protocol.packet.forge.Packet211TileDesc;
import net.md_5.bungee.protocol.packet.forge.Packet212GuiEvent;
...
public class Vanilla implements Protocol {
	...
	public Vanilla() {
		...
		/*
		 * classes[0xCE] = PacketCEScoreboardObjective.class; classes[0xCF] =
		 * PacketCFScoreboardScore.class; classes[0xD0] = PacketD0DisplayScoreboard.class;
		...
		classes[0xD3] = Packet211TileDesc.class;
		classes[0xD4] = Packet212GuiEvent.class;
	...
	public DefinedPacket read(short packetId, ByteBuf buf) {
		...
		try {
			if (buf.readerIndex() == start) { throw new RuntimeException("Unknown packet id " + packetId); }
		}
	...
	
	{
		...
		opCodes[0xFE] = new OpCode[] {}; // Should be byte, screw you too bitchy server
																			// admins!
		opCodes[0xFF] = new OpCode[] { STRING };
		
		// FORGE
		
		// Chickenchunks package
		/*
		 * opCodes[0xD3] = new OpCode[]{ BYTE,STRING, SHORT_BYTE };
		 */
		
		// Redpower packet
		/*
		 * opCodes[0xD3] = new OpCode[]{ };
		 */
	}
}
