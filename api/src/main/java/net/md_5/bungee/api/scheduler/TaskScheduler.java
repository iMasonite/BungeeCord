...
/** This interface represents a scheduler which may be used to queue, delay and execute
 * tasks in an asynchronous fashion. */
public interface TaskScheduler {
	
	/** Cancel a task to prevent it from executing, or if its a repeating task, prevent its
	 * further execution.
	 * 
	...
	/** Cancel all tasks owned by this plugin, this preventing them from being executed
	 * hereon in.
	 * 
	...
	/** Schedule a task to be executed asynchronously. The task will commence running as soon
	...
	/** Schedules a task to be executed asynchronously after the specified delay is up. The
	 * scheduled task will continue running at the specified interval. The interval will not
	 * begin to count down until the last task invocation is complete.
	 * 
	 * @param owner the plugin owning this task
	 * @param task the task to run
