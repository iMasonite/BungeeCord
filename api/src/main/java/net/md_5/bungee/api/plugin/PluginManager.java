...
/** Class to manage bridging between plugin duties and implementation duties, for example
 * event handling and plugin management. */
...
public class PluginManager {
	...
	private boolean enablePlugin(Map<PluginDescription, Boolean> pluginStatuses, Stack<PluginDescription> dependStack, PluginDescription plugin) {
		...
		if (status) {
			try {
				@SuppressWarnings("resource") URLClassLoader loader = new PluginClassloader(new URL[] { plugin.getFile().toURI().toURL() });
	...
	/** Dispatch an event to all subscribed listeners and return the event once it has been
	 * handled by these listeners.
	 * 
	...
	/** Register a {@link Listener} for receiving called events. Methods in this Object which
	 * wish to receive events must be annotated with the {@link Subscribe} annotation.
	 * 
	...
	public void registerListener(Plugin plugin, Listener listener) {
		for (Method method : listener.getClass().getDeclaredMethods()) {
			if (method.isAnnotationPresent(Subscribe.class)) {
				proxy.getLogger().log(Level.WARNING, "Listener " + listener + " has registered using depreceated subscribe annotation!" + " Please advice author to update to @EventHandler." + " As a server owner you may safely ignore this.", new Exception());
			}
		}
