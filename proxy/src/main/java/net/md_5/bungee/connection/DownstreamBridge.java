...
public class DownstreamBridge extends PacketHandler {
	...
	public void handle(PacketC9PlayerListItem playerList) throws Exception {
		...
		if (!con.getTabList().onListUpdate(playerList.getUsername(), playerList.isOnline(), playerList.getPing())) { throw new CancelSendSignal(); }
	}
	...
	public void handle(PacketFAPluginMessage pluginMessage) throws Exception {
		...
		if (bungee.getPluginManager().callEvent(event).isCancelled()) { throw new CancelSendSignal(); }
		
		if (pluginMessage.getTag().equals("MC|TPack") && con.getPendingConnection().getListener().getTexturePack() != null) { throw new CancelSendSignal(); }
		...
		if (pluginMessage.getTag().equals("BungeeCord")) {
			...
			// Check we haven't set out to null, and we have written data, if so reply back back
			// along the BungeeCord channel
			if (out != null) {
